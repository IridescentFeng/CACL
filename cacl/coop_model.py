...
class CustomCLIP(nn.Module):
    def __init__(self, args, classnames, clip_model):
        ...
        self.head = nn.Sequential(nn.Linear(dim_in, dim_in), nn.BatchNorm1d(dim_in), nn.ReLU(inplace=True),
                                  nn.Linear(dim_in, feat_dim)).to(dtype=self.dtype)
    ...
    def forward(self, image):
        ...
        #a = text_features.t()
        #b = self.head(text_features)
        centers_logits = F.normalize(self.head(text_features), dim=1)
        finall_image_features = F.normalize(self.head(image_features), dim=1)

        return logits, finall_image_features, centers_logits
